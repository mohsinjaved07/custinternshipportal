            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card shadow">
                        <h5 class="card-header bg-dark text-white text-center">Organization Information</h5>
                        <div class="card-body">
                            <p class="card-title font-weight-bold custFontColor">
                                Here are the organization list.
                            </p>
                            <hr class="my-4"/>
                            <label>Search Organization NTN</label>
                            <div class="input-group mb-2">
                                <div class="input-group-prepend">
                                    <div class="input-group-text"><i class="fas fa-search"></i></div>
                                </div>
                                <input type="text" class="form-control" id="myInput" onkeyup="myFunction()" placeholder="Search for NTN no..."/><br/>
                            </div>
                            <div class="scroller">
                                <table class="table" id="myTable">
                                    <thead class="thead-dark">
                                        <tr>
                                            <th scope="col">Organization NTN No</th>
                                            <th scope="col">Organization Name</th>
                                            <th scope="col">Organization Email</th>
                                            <th scope="col">Organization Contact</th>
                                            <th scope="col">Organization Address</th>
                                            <th scope="col">Supervisor's Lists</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($org as $o)
                                        <tr>
                                            <td>{{ $o->organization_ntn_no }}</td>
                                            <td>{{ $o->organization_name }}</td>
                                            <td>{{ $o->organization_email }}</td>
                                            <td>{{ $o->organization_contact }}</td>
                                            <td>{{ $o->organization_address }}</td>
                                            <td class="text-center">
                                                <a href="{{ url('/coordinator/'.$o->organization_ntn_no.'/supervisors') }}" class="btn btn-success" role="button">
